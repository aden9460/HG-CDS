...
def main():
    ...
    for epoch in range(args.start_epoch + 1, args.epochs):
        ...
        with torch.no_grad():
            ...
            # wandb.log("loss", {"val_loss": val_loss, "val_prec1": val_prec1}, epoch)
            if args.record:
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if training:
            ...
            # print(inputs.size())
            outputs, feat_list = model(inputs)
            outputs = outputs[:bsz]

            loss = criterion(outputs, target)
            if args.record:
                wandb.log({"CE_loss": loss, "epoch": epoch})
        ...
        if not training:
            with torch.no_grad():
                # bsz = target.size(0)  # 128     labels:torch.Size([128])    batchsize=128
                inputs, target = inputs.cuda(), target.cuda()
        ...
        if i % args.print_freq == 0:
            logging.info('{phase} - Epoch: [{0}][{1}/{2}]\t'
                ...
                data_time=data_time, c_loss=c_losses, loss=losses,
                top1=top1, top5=top5))
    ...
    for name, param in model.named_parameters():
        if param.requires_grad and 'bias' not in name:  # 排除偏置项
            layer_name = name.split('.')[0]  # 提取层名称
            if layer_name not in average_gradients:
                average_gradients[layer_name] = []
            average_gradients[layer_name].append(param.grad.abs().mean().item())
    print(print({layer: sum(gradients) / len(gradients) for layer, gradients in average_gradients.items()}))
