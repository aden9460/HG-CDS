...
def distillation(student_output, teacher_output, temperature=4.0):
    """Knowledge distillation loss using KL divergence.

    Args:
        student_output: Logits from the student (binary) model, shape [B, C].
        teacher_output: Logits from the teacher (full-precision) model, shape [B, C].
        temperature: Softening temperature T (default: 4.0).

    Returns:
        Scalar KL-divergence distillation loss scaled by T^2.
    """
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        Returns:
            A loss scalar.
        """
        # features = torch.sign(features)  ####二值化输入
