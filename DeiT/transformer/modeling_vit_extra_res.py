...
class ViTSelfOutput(nn.Module):
    """
    Attention输出层 - 实现论文公式(2)的BiFC（二值全连接层）

    论文公式(2):
        BiFC(X) = RPReLU(BN(Y(X)) + R(X))

    其中:
        - X: 输入特征
        - Y(X): 通过QuantizeLinear计算的输出（公式1）
        - BN: 批归一化层
        - R(X): 残差函数（根据输入输出维度选择，见公式3）
        - RPReLU: 激活函数

    注: layernorm已被替换为batchnorm以加速推理和训练
    """
...
class ViTIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        linear_out = self.norm(self.dense(hidden_states + self.move))


        residual = torch.concat((hidden_states,
                               hidden_states,
                               hidden_states,
                               hidden_states), dim=-1)

        # BN(Y(X)) + R(X)
        out = linear_out + residual
...
class ViTOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        residual = self.pooling(hidden_states)
