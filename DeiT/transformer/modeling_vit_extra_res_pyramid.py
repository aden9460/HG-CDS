...
@dataclass
class ImageClassifierOutputWithAuxiliary(ImageClassifierOutput):
    """
    Extended ImageClassifierOutput that includes auxiliary features for CDS.
    """
...
from .cds_modules import ViTAuxiliaryClassifier, MultiScaleViTAuxiliaryClassifier  # CDS support
...
class ViTPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 使用配置文件中的 patch_size，而不是硬编码为 4
        # patch_size = 4  # 旧版本：硬编码
        patch_size = config.patch_size  # 新版本：从配置读取
...
class ViTEncoder(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        init_patch_size = config.image_size // config.patch_size
        stage_sizes = [init_patch_size]
        for _ in range(len(config.hidden_size) - 1):
            stage_sizes.append(stage_sizes[-1] // 2)


        num_stages = len(config.hidden_size)

        if config.some_fp:
            if num_stages >= 2:
                self.patch_embed1 = PatchEmbed(stage_sizes[0], in_dim=config.hidden_size[0], out_dim=config.hidden_size[1], config=config)
            if num_stages >= 3:
                self.patch_embed2 = PatchEmbed(stage_sizes[1], in_dim=config.hidden_size[1], out_dim=config.hidden_size[2], config=config)
            if num_stages >= 4:
                self.patch_embed3 = PatchEmbed(stage_sizes[2], in_dim=config.hidden_size[2], out_dim=config.hidden_size[3], config=config)
        else:
            if num_stages >= 2:
                self.patch_embed1 = BinaryPatchEmbed(stage_sizes[0], in_dim=config.hidden_size[0], out_dim=config.hidden_size[1], config=config)
            if num_stages >= 3:
                self.patch_embed2 = BinaryPatchEmbed(stage_sizes[1], in_dim=config.hidden_size[1], out_dim=config.hidden_size[2], config=config)
            if num_stages >= 4:
                self.patch_embed3 = BinaryPatchEmbed(stage_sizes[2], in_dim=config.hidden_size[2], out_dim=config.hidden_size[3], config=config)
    ...
    def forward(
        ...
        stage_features = [] if return_auxiliary else None  # Collect features for CDS
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == self.depths[0] - 1:
                if return_auxiliary:
                    stage_features.append(hidden_states)  # Stage 0 features
                if hasattr(self, 'patch_embed1'):
                    hidden_states = self.patch_embed1(hidden_states)
            elif num_stages >= 3 and i == self.depths[0] + self.depths[1] - 1:
                if return_auxiliary:
                    stage_features.append(hidden_states)  # Stage 1 features
                if hasattr(self, 'patch_embed2'):
                    hidden_states = self.patch_embed2(hidden_states)
            elif num_stages >= 4 and i == self.depths[0] + self.depths[1] + self.depths[2] - 1:
                if return_auxiliary:
                    stage_features.append(hidden_states)  # Stage 2 features
        ...
        # Collect final stage features (Stage 2 for 3-stage, Stage 3 for 4-stage)
        if return_auxiliary:
            stage_features.append(hidden_states)

        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)

        if not return_dict:
            result = tuple(v for v in [hidden_states, all_hidden_states, all_self_attentions] if v is not None)
            if return_auxiliary:
                result = result + (stage_features,)
            return result
        ...
        # Attach stage_features as extra attribute
        if return_auxiliary:
            output.stage_features = stage_features
        return output
...
class ViTModel(nn.Module):
    ...
    def __init__(self, config: ViTConfig):
        ...
        if hasattr(config, 'hidden_size') and isinstance(config.hidden_size, list):

            layernorm_size = config.hidden_size[-1]  # 512
        else:
            layernorm_size = config.hidden_size
        self.layernorm = config.norm_layer(layernorm_size, eps=config.layer_norm_eps)
    ...
    def forward(
        ...
        sequence_output = encoder_outputs[0]  # [B, final_num_patches, final_hidden_size]
        ...
        if not return_dict:
            ...
            result = head_outputs + encoder_outputs[1:] 
            if return_auxiliary and hasattr(encoder_outputs, 'stage_features'):
                result = result + (encoder_outputs.stage_features,)
            return result
        ...
        if return_auxiliary and hasattr(encoder_outputs, 'stage_features'):
            output.stage_features = encoder_outputs.stage_features
        return output
